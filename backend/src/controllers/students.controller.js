...
export async function createStudent(req, res) {
  const { full_name, grade_level, class_name, birth_date, school_name, guardian_name, guardian_email } = req.body;
  const result = await query(
    `
      INSERT INTO students (full_name, grade_level, class_name, birth_date, school_name, guardian_name, guardian_email, tenant_id)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      RETURNING *
    `,
    [full_name, grade_level || null, class_name || null, birth_date || null, school_name || null, guardian_name || null, guardian_email || null, req.user.tenantId]
  );
...
export async function updateStudent(req, res) {
  ...
  const result = await query(
    `
      ...
      RETURNING *
    `,
    [req.params.id, req.user.tenantId, full_name, grade_level, class_name, birth_date, school_name, guardian_name, guardian_email]
  );
  if (!result.rows[0]) return res.status(404).json({ error: 'Estudante não encontrado.' });
  res.json(result.rows[0]);
}
